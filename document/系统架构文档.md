...
| core/sculpt_adapter.py | 变形与雕刻操作的 bpy 适配（简易变形、网格雕刻、软选择变形、曲线变形、网格细分、投射变形） |
...
| simple_deform | 简易变形（弯曲、锥化、扭曲、拉伸） | 高 |
| mesh_sculpt | 网格雕刻（推、拉、平滑、膨胀） | 高 |
| soft_transform | 软选择变形（基于影响半径的变形） | 高 |
| curve_deform | 曲线变形（沿曲线弯曲对象） | 高 |
| subdivide_mesh | 网格细分（增加可编辑顶点密度） | 高 |
| save_project | 保存项目 | 中 |
| open_project | 打开项目 | 中 |
...
// 简易变形工具
interface SimpleDeformParams {
  object_id: string;
  operation: 'bend' | 'taper' | 'twist' | 'stretch';
  angle?: number;         // bend/twist 的角度（度）
  factor?: number;        // taper/stretch 的因子 (-2.0 ~ 2.0)
  axis?: 'x' | 'y' | 'z'; // 变形轴
  origin?: [number, number, number]; // 变形原点
  limits?: [number, number]; // 变形限制（0.0~1.0）
}

interface SimpleDeformResult {
  success: boolean;
  object_id: string;
  operation: string;
  vertex_count: number;
...
// 网格雕刻工具
interface MeshSculptParams {
  object_id: string;
  operation: 'push' | 'pull' | 'smooth' | 'inflate' | 'flatten' | 'pinch';
  center: [number, number, number];  // 操作中心点（世界坐标）
  radius: number;                     // 影响半径
  strength: number;                   // 操作强度 (0.0~1.0)
  direction?: 'normal' | 'view' | 'x' | 'y' | 'z'; // 操作方向
  symmetry?: 'x' | 'y' | 'z' | 'none'; // 对称轴
  iterations?: number;                // 迭代次数（smooth用）
}

interface MeshSculptResult {
  success: boolean;
  object_id: string;
  operation: string;
...
// 软选择变形工具
interface SoftTransformParams {
  object_id: string;
  selection_center: [number, number, number]; // 选择中心
  selection_radius: number;                    // 选择半径
  falloff_type?: 'smooth' | 'sphere' | 'root' | 'linear' | 'constant';
...
// 曲线变形工具
interface CurveDeformParams {
  object_id: string;
  curve_id: string;       // 用作变形路径的曲线对象ID
  axis?: 'x' | 'y' | 'z'; // 变形轴
}

interface CurveDeformResult {
  success: boolean;
  object_id: string;
  curve_id: string;
}
...
interface SubdivideMeshParams {
  object_id: string;
  levels: number;          // 细分层级 (1-6)
  smoothness?: number;     // 平滑度 (0.0~1.0)
  boundary?: 'none' | 'smooth'; // 边界处理
}

interface SubdivideMeshResult {
  success: boolean;
  object_id: string;
...
// 投射变形工具
interface ShrinkwrapParams {
  object_id: string;
  target_id: string;       // 目标对象ID
  mode?: 'nearest_surface' | 'projection' | 'nearest_vertex';
  offset?: number;         // 偏移距离
  substeps?: number;       // 子步数
}
