...
gantt
    ...
    import_model/export_model :b5, after b4, 1d
    check_model/repair_model  :b6, after b5, 1.5d
    detect_overhangs          :b7, after b6, 1d
    optimize_orientation       :b8, after b7, 1d
    rotate_to_orientation/set_shrinkage_compensation :b9, after b8, 0.5d
    validate_printability/export_slicer_preset :b10, after b9, 1d
    list_objects/scene_info   :b11, after b10, 0.5d
    单元测试编写              :b12, after b11, 1d
    里程碑 M2                 :milestone, after b12, 0d

    section 阶段三：文件管理与状态同步
    save/open_project         :c1, after b12, 1d
    状态管理器完善            :c2, after c1, 1d
    事件通知与性能优化        :c3, after c2, 1d
    集成测试                  :c4, after c3, 1d
    里程碑 M3                 :milestone, after c4, 0d

    section 阶段四：材质与渲染
    set_material              :d1, after c4, 1.5d
    render_scene              :d2, after d1, 1.5d
    集成测试                  :d3, after d2, 1d
    里程碑 M4                 :milestone, after d3, 0d

    section 阶段五：文档与优化
    文档编写                  :e1, after d3, 1.5d
    代码优化与重构            :e2, after e1, 0.5d
    全面测试与 Bug 修复       :e3, after e2, 1d
    里程碑 M5                 :milestone, after e3, 0d
```
...
```mermaid
flowchart TD
    M1["🏁 M1: 基础架构\n阶段一 (2-3天)"] --> M2["🏁 M2: 3D 打印建模\n阶段二 (5-6天)"]
    M2 --> M3["🏁 M3: 文件管理与同步\n阶段三 (2-3天)"]
    M2 --> M4["🏁 M4: 材质与渲染\n阶段四 (3-4天)"]
    M3 --> M5["🏁 M5: 文档与优化\n阶段五 (2-3天)"]
    M4 --> M5

    M1 -->|出: MCP 服务 + Blender 插件| M2
    M2 -->|出: 核心建模工具集| M3
    M2 -->|出: 核心建模工具集| M4
    M3 -->|出: 完整文件管理| M5
    M4 -->|出: 材质与渲染功能| M5
